...
void runcmd(struct cmd *cmd)
{
    ...
    case '|':
        ...
        pid_t pid = fork1();        // verica a qual processo pertence
        if (pid == 0)
        {
        ...
        else
        {
            ...
            else
            {
                ...
                close(pipefd[0]);
                close(pipefd[1]);
                /// aguarda o fim da execução dos processos filhos
                wait(NULL);
                wait(NULL);
            }
        }
...
struct cmd *parsecmd(char *s)
{
    ...
    if (s != es)
    { // A entrada não foi interamente consumida
...
struct cmd *parseexec(char **ps, char *es)
{
    ...
    while (!peek(ps, es, "|"))
    { // verifica se o comando não é um pipe
