...
if ! tests/test0.sh $srcfn ; then
    ecnt=$(( $ecnt + 1 ))
    echo "Test 0 failed: Code compilation failed or produced warnings"
fi

# Test 1: Simple ls command
if ! tests/test1.sh $srcfn ; then
    ecnt=$(( $ecnt + 1 ))
    echo "Test 1 failed: Simple ls command failed"
fi

# Test 2: Sequence of simple ls commands
if ! tests/test2.sh $srcfn ; then
    ecnt=$(( $ecnt + 1 ))
    echo "Test 2 failed: Sequence of simple ls commands failed"
fi

# Test 3: Output redirection command
if ! tests/test3.sh $srcfn ; then
    ecnt=$(( $ecnt + 1 ))
    echo "Test 3 failed: Output redirection command failed"
fi

# Test 4: Input redirection command
if ! tests/test4.sh $srcfn ; then
    ecnt=$(( $ecnt + 1 ))
    echo "Test 4 failed: Input redirection command failed"
fi

# Test 5: Input/output redirection command
if ! tests/test5.sh $srcfn ; then
    ecnt=$(( $ecnt + 1 ))
    echo "Test 5 failed: Input/output redirection command failed"
fi

# Test 6: Pipe command
if ! tests/test6.sh $srcfn ; then
    ecnt=$(( $ecnt + 1 ))
    echo "Test 6 failed: Pipe command failed"
fi

# Test 7: Sequence of pipe commands
if ! tests/test7.sh $srcfn ; then
    ecnt=$(( $ecnt + 1 ))
    echo "Test 7 failed: Sequence of pipe commands failed"
fi

# Test 8: Sequence of pipe commands with output redirection
if ! tests/test8.sh $srcfn ; then
    ecnt=$(( $ecnt + 1 ))
    echo "Test 8 failed: Sequence of pipe commands with output redirection failed"
fi
